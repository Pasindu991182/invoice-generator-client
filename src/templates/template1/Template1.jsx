...
const Template1 = ({ data }) => {
    return (
        <div className="template1 border rounded mx-auto my-5 px-sm-4 py-4 w-1000">
            {/* Header Section */}
            <div className={"row mb-4"}>
                <div className="col-md-6 mb-3 mb-md-0">
                    {data.CompanyLogo && (
                        <div className="mb-2">
                            <img src={data.CompanyLogo} alt="Company" width={98} />
                        </div>
                    )}
                ...
                <div className="col-md-6 text-start text-md-end">
                    ...
                    <div className="d-flex flex-column flex-md-row justify-content-md-end gap-2 gap-md-4">
                        <div className="w-100 w-50 mb-3 mb-md-0">
                            <p className="mb-1">
                    ...
                    <table className="table table-bordered">
                        ...
                        {data.items && data.items.map((item, index) => (
                            <tr key={index}>
                                ...
                                <td className="p-2 text-end">LKR{Number(item.amount)?.toFixed(2)}</td>
                                <td className="p-2 text-end">LKR{(Number(item.qty) * Number(item.amount)).toFixed(2)}</td>
                            </tr>
                        ))}
                        </tbody>
                    </table>
            ...
            {(data.accountName || data.accountNumber || data.accountIfsCode) && (
                <div className="mt-4">
                    ...
                    {data.accountIfsCode && (
                        <p className="mb-0">
                            <strong>Branch code/ IfsCode: </strong>{data.accountIfsCode}
                        </p>
                    )}
                </div>
            )}
            ...
            {data.notes && (
                <div className="mt-4">
                    <h3 className="mb-2 billing-title">Notes & Remarks</h3>
